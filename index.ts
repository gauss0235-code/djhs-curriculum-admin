...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "password") {
      ...
      // access_token이 응답에 있으면 = 동일 비밀번호로 로그인 성공
      try {
        ...
        console.log(`[password compare] status=${loginResp.status}, has_token=${!!loginData?.access_token}`);
        
        // access_token이 발급되면 = 이전 비밀번호와 동일
